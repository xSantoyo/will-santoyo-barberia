              ...
              {["Turno reservado en línea: sin filas ni llamadas", "Fades, barba y diseño — técnica y producto de primera", "Tu código de gestión en pantalla: consulta o cancela cuando quieras"].map(
                (item) => (
                  <li key={item} className="flex items-center gap-3">
                    <Scissors size={16} className="shrink-0 text-copper" />
                    {item}
                  </li>
                ),
              )}
...
export function Services({ services }: { services: ServicePublic[] }) {
  return (
    <section
      id="servicios"
      className="grain relative overflow-hidden border-y border-edge bg-coal"
    >
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-24 sm:py-28">
        ...
        <StaggerGroup className="mt-12 grid gap-x-10 gap-y-1 sm:grid-cols-2">
          {services.map((service) => (
            <StaggerItem key={service.id}>
              <div className="group flex items-baseline justify-between gap-4 border-b border-edge px-2 py-5 transition-[border-color,background-color] duration-200 ease-[var(--ease-out)] hover:border-copper/50 hover:bg-copper/[0.04]">
                <div>
                  <h3 className="text-lg text-chalk transition-colors group-hover:text-copper">
                    {service.name}
                  </h3>
                  <p className="data text-sm text-smoke">{service.duration_min} min</p>
                </div>
                <p className="data whitespace-nowrap text-xl font-semibold text-copper transition-transform duration-150 ease-[var(--ease-out)] group-hover:scale-105">
        ...
        <Reveal delay={0.2}>
          <div className="mt-12 flex flex-wrap items-center gap-5">
            <Link
              href="/agendar"
              className="display inline-block rounded-sm bg-copper px-8 py-4 text-lg text-on-copper transition-transform duration-150 ease-[var(--ease-out)] hover:scale-[1.02] active:scale-[0.97]"
            >
              Reservar con Will
            </Link>
          </div>
...
function Stars({ average, count }: { average: number; count: number }) {
  return (
    <span className="data flex items-center gap-1 text-xs text-copper">
      <Star size={12} className="fill-copper" />
      {average.toFixed(1)}
...
export function Trayectoria({ data }: { data: TrayectoriaType | null }) {
  ...
  return (
    <section className="relative overflow-hidden border-y border-edge">
      <div className="relative mx-auto max-w-6xl px-5 py-16">
        <StaggerGroup className="grid gap-8 text-center sm:grid-cols-3">
          {cifras.map((cifra) => (
            <StaggerItem key={cifra.etiqueta}>
              <p className="display text-5xl text-copper sm:text-6xl">{cifra.valor}</p>
              <p className="data mt-2 text-[11px] uppercase tracking-[0.25em] text-smoke">
                {cifra.etiqueta}
              </p>
            </StaggerItem>
          ))}
...
export function Gallery({ items }: { items: MediaAsset[] }) {
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-24 sm:py-28">
        <RazorDivider className="mb-10 !px-0" />
        <SectionTitle kicker="El oficio" title="Cómo se trabaja acá" />
        {items.length === 0 ? (
          <Reveal delay={0.1}>
            ...
            <StaggerGroup className="mt-12 columns-2 gap-4 md:columns-3 [&>div]:mb-4">
              {items.map((item) => (
                <StaggerItem key={item.id}>
                  <div className="group overflow-hidden rounded-[var(--radius-card)] border border-edge transition-colors duration-200 hover:border-copper/40">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={mediaUrl(item.url) ?? ""}
                      alt="Ambiente de barbería"
                      loading="lazy"
                      className="w-full transition-transform duration-200 ease-[var(--ease-out)] group-hover:scale-[1.02]"
                    />
                  </div>
                </StaggerItem>
              ))}
            </StaggerGroup>
            {/* Honestidad: estas imágenes son de referencia, no cortes de Will */}
            <Reveal delay={0.15}>
              <p className="mt-8 text-sm text-smoke/70">
                Imágenes de referencia del oficio. Las fotos de mis propios
                cortes van aquí en cuanto las suba —{" "}
                <a
                  href={NEGOCIO.instagram}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-copper underline-offset-4 transition-colors hover:underline"
                >
                  mientras tanto están en mi Instagram
                </a>
                .
              </p>
            </Reveal>
    ...
    <section id="resenas" className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-24 sm:py-28">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <Reveal delay={0.1}>
            <p className="data text-right text-sm text-smoke">
              <span className="stamped text-4xl text-copper">
                {data.overall.average?.toFixed(1)}
              </span>
              <span className="ml-2 text-copper">★</span>
          ...
          {data.items.slice(0, 6).map((review, i) => (
            <StaggerItem key={`${review.date_local}-${i}`}>
              <figure className="surface surface h-full p-5">
                ...
                {review.comment && (
                  <blockquote className="mt-3 text-sm leading-relaxed text-chalk">
                    “{review.comment}”
                  </blockquote>
                )}
                <figcaption className="data mt-4 text-[11px] uppercase tracking-wider text-smoke">
        ...
        <StaggerGroup className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {products.map((product) => (
            <StaggerItem key={product.id}>
              <div className="surface group h-full border border-edge bg-night transition-[transform,border-color] duration-200 ease-[var(--ease-out)] hover:-translate-y-1 hover:border-copper/40">
                <div className="grain relative aspect-square overflow-hidden bg-ash">
                  {product.photo_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                ...
                <div className="p-4">
                  <h3 className="text-chalk">{product.name}</h3>
                  {product.description && (
                    <p className="mt-1 text-xs text-smoke">{product.description}</p>
                  )}
                  <p className="data mt-2 text-lg font-semibold text-copper">
...
export function Location({ tenant }: { tenant: TenantPublic }) {
  const copper = tenant.brand_config;
  ...
  return (
    <section id="ubicacion" className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-24 sm:py-28">
        ...
        <div className="mt-12 grid gap-10 md:grid-cols-2">
          <Reveal>
            <div className="space-y-5 text-smoke">
              <p className="flex items-start gap-3">
                <MapPin className="mt-1 shrink-0 text-copper" size={18} />
                <span>
                  {NEGOCIO.calle}
                  <br />
              ...
              <p className="flex items-center gap-3">
                <svg viewBox="0 0 24 24" width="18" height="18" className="shrink-0 fill-copper">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="min-h-11 leading-[2.75rem] transition-colors duration-150 hover:text-copper"
                >
                  {NEGOCIO.telefono}{" "}
                  <span className="data text-xs text-smoke/70">— escríbeme</span>
                </a>
              </p>
              <a
                href={MAPS_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="display inline-block rounded-sm border border-copper px-6 py-3.5 text-copper transition-[background-color,color,transform] duration-150 ease-[var(--ease-out)] hover:bg-copper hover:text-on-copper active:scale-[0.98]"
              >
          ...
          <Reveal delay={0.15}>
            <div className="surface grain relative border border-edge bg-coal p-6">
              <p className="mb-4 flex items-center gap-2 text-sm uppercase tracking-widest text-copper">
              ...
              <ul className="divide-y divide-edge">
                {hours.map(({ key, label, block }) => (
                  <li
                    key={key}
                    className="flex justify-between py-2.5 text-sm transition-colors hover:bg-copper/[0.04]"
                  >
                    <span className="text-smoke">{label}</span>
                    <span className={`data ${block ? "text-chalk" : "text-brick"}`}>
...
export function Footer({ tenant }: { tenant: TenantPublic }) {
  const copper = tenant.brand_config;
  const socials = [
    { href: copper.instagram as string | undefined, icon: Instagram, label: "Instagram" },
    { href: copper.facebook as string | undefined, icon: Facebook, label: "Facebook" },
    { href: copper.tiktok as string | undefined, icon: Music2, label: "TikTok" },
  ].filter((s) => s.href);

  return (
    <footer className="relative overflow-hidden border-t border-edge bg-coal">
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col items-center gap-6 px-5 py-14 text-center">
        ...
        <div className="flex gap-6">
          {socials.map(({ href, icon: Icon, label }) => (
            <a
              ...
              rel="noopener noreferrer"
              aria-label={label}
              className="-m-2 p-2 text-smoke transition-[transform,color] duration-150 ease-[var(--ease-out)] hover:scale-105 hover:text-copper"
            >
              <Icon size={22} />
            </a>
          ))}
        ...
        <p className="text-xs text-smoke/60">
          ...
          {" · "}
          <Link href="/admin" className="transition-colors hover:text-copper">

...
export default function ClientProfileModal({
  ...
  return (
    ...
    >
      {error && (
        <div className="mb-4 rounded-sm border border-brick bg-brick/15 px-3 py-2 text-sm">
          {error}
        </div>
      )}
      {!profile && !error && (
        <div className="flex justify-center py-10 text-copper">
          <Loader2 className="animate-spin" size={28} />
        </div>
      )}
      {profile && (
        <div className="space-y-5">
          <p className="data text-xs text-smoke">{profile.phone}</p>
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            {[
              ...
              <div key={stat.label} className="rounded-sm border border-edge bg-night px-2 py-2.5">
                <p className="data text-lg font-semibold text-chalk">{stat.value}</p>
                <p className="data text-[10px] uppercase tracking-wider text-smoke">
                  {stat.label}
                </p>
              </div>
            ))}
          ...
          <div className="surface surface p-4">
            <p className="data flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-copper">
              ...
              {profile.loyalty.earned_rewards > 0 && (
                <span className="ml-auto flex items-center gap-1 text-copper">
                  <Star size={11} className="fill-copper" />
                  {profile.loyalty.earned_rewards} por redimir
                </span>
              )}
            </p>
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-ash">
              <div
          ...
          {/* Notas de estilo */}
          <div>
            <p className="data mb-2 flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-smoke">
              <NotebookPen size={12} className="text-copper" /> Notas de estilo
            </p>
            ...
            <ul className="mt-3 space-y-2">
              {profile.notes.map((item) => (
                <li
                  key={item.id}
                  className="group flex items-start justify-between gap-3 rounded-sm border border-edge bg-night px-3 py-2.5"
                >
                  <div>
                    <p className="text-sm text-chalk">{item.note}</p>
                    <p className="data mt-1 text-[10px] uppercase tracking-wider text-smoke">
                  ...
                  {(auth?.role === "admin" || auth?.username === item.author_name) && (
                    <button
                      ...
                      className="hidden shrink-0 text-brick group-hover:block"
          ...
          {/* Historial reciente */}
          <div>
            <p className="data mb-2 text-[11px] uppercase tracking-[0.25em] text-smoke">
              Últimos turnos
            </p>
            <ul className="max-h-44 space-y-1.5 overflow-y-auto pr-1">
              {profile.recent.map((appointment) => (
                <li
                  key={appointment.id}
                  className="flex items-center justify-between gap-2 rounded-sm bg-night px-3 py-2 text-xs"
                >
                  <span className="truncate text-smoke">
                    <span className="data mr-1.5 text-copper">{appointment.date_local}</span>

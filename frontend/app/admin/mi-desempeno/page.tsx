            ...
            {RANGES.map((range) => (
              <button
                ...
                className={`data rounded-sm px-3 py-1.5 text-xs uppercase tracking-wider transition-colors ${
                  days === range.days ? "bg-gold text-ink" : "text-bone-2 hover:text-bone"
                }`}
              >
                {range.label}
              </button>
      ...
      {!stats ? (
        ...
        <>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">
            <StatCard
              icon={<CalendarClock size={13} className="text-gold" />}
              label="Aún por atender hoy"
              value={String(stats.upcoming_today)}
            />
            <StatCard
              icon={<UserX size={13} className={stats.no_show_count > 0 ? "text-wine" : "text-gold"} />}
              label="No-shows"
              value={String(stats.no_show_count)}
              alert={stats.no_show_count > 0}
            />
            <StatCard
              icon={<UserX size={13} className="text-gold" />}
              label="Cancelados"
              value={String(stats.cancelled_count)}
            />
          </div>

          <section className="mt-8 max-w-xl rounded-sm border border-ink-3 bg-ink-2 p-5">
            <h2 className="display mb-4 text-2xl text-bone">Mis servicios más pedidos</h2>
                ...
                {stats.top_services.map((service, index) => {
                  ...
                  return (
                    <li key={service.name} className="flex items-center gap-3">
                      <span className="data w-5 text-xs text-bone-2">{index + 1}.</span>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-sm text-bone">{service.name}</p>
                          <span className="data text-xs text-gold">{service.count}</span>
                        </div>
                        <div className="mt-1 h-1 rounded-full bg-ink">
                          <div
          ...
          <p className="mt-6 max-w-xl text-xs text-bone-2">
...
function StatCard({
  ...
  return (
    <div className={`plate clip-corner p-4 ${alert ? "border-wine/60" : ""}`}>
      ...
      <p
        className={`data mt-1 text-2xl font-semibold ${
          alert ? "text-wine" : gold ? "text-gold" : "text-bone"
        }`}
      >
        {value}
      </p>
      {hint && <p className="data mt-0.5 text-[11px] text-bone-2">{hint}</p>}
    </div>
  );
}

...
const TONE_CLASS = {
  info: "border-ink-3 text-bone-2",
  warn: "border-gold/50 text-gold",
  bad: "border-wine/60 text-wine",
};
...
export default function SecurityPage() {
  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-wrap gap-1 rounded-sm border border-ink-3 p-1">
        {FILTERS.map((filter) => (
          <button
            key={filter.kind}
            onClick={() => setKind(filter.kind)}
            className={`data rounded-sm px-3 py-1.5 text-xs uppercase tracking-wider transition-colors ${
              kind === filter.kind ? "bg-gold text-ink" : "text-bone-2 hover:text-bone"
            }`}
          >
            {filter.label}
          </button>
      ...
      {!events ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-sm border border-dashed border-ink-3 p-12 text-center">
          <ShieldAlert size={28} className="text-bone-2/50" />
          <p className="text-sm text-bone-2">Sin eventos registrados. Todo tranquilo.</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-sm border border-ink-3">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead className="bg-ink-2">
              <tr className="data text-[11px] uppercase tracking-wider text-bone-2">
            ...
            <tbody>
              {events.map((event) => {
                ...
                return (
                  <tr key={event.id} className="border-t border-ink-3">
                    <td className="data whitespace-nowrap px-4 py-2.5 text-xs text-bone-2">
                    ...
                    <td className="data px-4 py-2.5 text-xs text-bone">
                      {event.username ?? "—"}
                    </td>
                    <td className="data px-4 py-2.5 text-xs text-bone-2">{event.ip ?? "—"}</td>
                    <td className="data max-w-[280px] truncate px-4 py-2.5 text-xs text-bone-2">

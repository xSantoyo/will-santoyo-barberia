    ...
    <main className="min-h-svh pt-10">
      <div className="mx-auto max-w-md px-5">
        ...
        <form onSubmit={search} className="mt-10 space-y-5">
          {error && (
            <div className="rounded-sm border border-wine bg-wine/15 px-4 py-3 text-sm text-bone">
              {error}
            </div>
          )}
          ...
          <label className="block">
            <span className="mb-1.5 block text-sm text-bone-2">Código del turno</span>
            <input
              value={code}
              ...
              maxLength={8}
              required
              className="focus-gold w-full rounded-sm border border-ink-3 bg-ink-2 px-4 py-3 uppercase tracking-[0.3em] text-bone placeholder:tracking-normal placeholder:text-bone-2/50"
            />
          </label>
          <button
            type="submit"
            disabled={loading}
            className="display flex w-full items-center justify-center gap-2 rounded-sm bg-gold px-6 py-3.5 text-lg text-ink transition-transform enabled:hover:scale-[1.02] disabled:opacity-60"
          >

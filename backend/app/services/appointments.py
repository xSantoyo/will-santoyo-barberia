...
from .professional import get_professional
...
def generate_manage_code(db: Session, length: int = 8) -> str:
    """8 caracteres de un alfabeto de 31 (sin ambiguos) ≈ 8.5e11 combinaciones:
    imposible de adivinar o recorrer con el rate limit de consultas por código.
    (Se subió de 6 a 8 en la auditoría de seguridad; los códigos viejos de 6
...
def next_daily_number(db: Session, tenant: Tenant, professional_id: int, day: date) -> int:
...
def _overlap_exists(
    db: Session, professional_id: int, starts_at: datetime, ends_at: datetime,
...
def _validate_slot(
    ...
    *,
    ...
    if sched is None or is_time_off(db, professional.id, day):
        raise BookingError(f"{professional.name} no trabaja ese día.", 409, "day_off")
    ...
    if starts_at < work_start or ends_at > work_end:
        raise BookingError(
            f"El horario está fuera de la jornada de {professional.name} "
...
def create_appointment(
    ...
    *,
    ...
    appointment = Appointment(
        ...
        ends_at=ends_at,
        status=status,
        daily_number=next_daily_number(db, tenant, professional.id, data.date),
...
def reschedule_appointment(
    ...
    *,
    ...
    tz = ZoneInfo(tenant.timezone)
    day_changed = (
        starts_at.astimezone(tz).date() != appointment.starts_at.astimezone(tz).date()
    )
    appointment.starts_at = starts_at
    appointment.ends_at = ends_at
    if day_changed:
        appointment.daily_number = next_daily_number(db, tenant, professional.id, new_date)
    try:
        db.commit()
    except IntegrityError:
        db.rollback()
...
def create_walk_in(
    ...
    *,
    ...
    hoy y entra a La Fila con su número y código."""
    ...
    if next_slot is None:
        raise BookingError(
            f"No queda espacio hoy en la agenda de {professional.name}.", 409, "full"
        )
    ...
    appointment = Appointment(
        ...
        ends_at=ends_at,
        status="confirmado",
        daily_number=next_daily_number(db, tenant, professional.id, today),
        manage_code=generate_manage_code(db),
        notes="Walk-in",
    )
    for service in services:
...
def create_group_appointment(db: Session, tenant: Tenant, data) -> list[Appointment]:
    ...
    base_number = next_daily_number(db, tenant, professional.id, data.date)
    tz = ZoneInfo(tenant.timezone)
...
def rebook_appointment(db: Session, tenant: Tenant, appointment: Appointment,
    ...
    data = BookingCreate(
        ...
        customer_email=appointment.customer_email,  # hereda la copia por correo
    )

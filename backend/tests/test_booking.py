...
def _get_slot(client, barber_id: int, day, service_ids: list[int]) -> list[str]:
    response = client.post(
        f"{BASE}/availability",
        json={"barber_id": barber_id, "date": day.isoformat(), "service_ids": service_ids},
    )
...
def test_tenant_info(client):
    ...
    data = response.json()
    assert data["name"] == "Bad Boys Barbershop"
    assert data["timezone"] == "America/Bogota"


def test_barbers_and_services_listed(client):
    barbers = client.get(f"{BASE}/barbers").json()
    services = client.get(f"{BASE}/services").json()
    assert len(barbers) == 3
    assert len(services) == 6
    classic = next(s for s in services if s["name"] == "Corte clásico")
    assert classic["price_cop"] == 30000


def test_full_booking_flow(client, db, barbers):
    barber = barbers[0]
    day = next_working_date(barber)
    services = client.get(f"{BASE}/services").json()
    service_id = services[0]["id"]

    availability = _get_slot(client, barber.id, day, [service_id])
...
def test_daily_number_increments_per_barber(client, barbers):
    barber = barbers[1]
    day = next_working_date(barber, weeks_ahead=1)
    services = client.get(f"{BASE}/services").json()
    ...
    first = client.post(
        f"{BASE}/appointments",
        json={
            "barber_id": barber.id, "service_ids": [service_id],
...
def test_multiple_services_extend_duration(client, barbers):
    """Corte + barba (60') + color (90') = 150 minutos: bloquea 10 slots de 15'."""
    barber = barbers[2]
    day = next_working_date(barber, weeks_ahead=2)
    services = client.get(f"{BASE}/services").json()
    combo = [services[1]["id"], services[5]["id"]]  # 60 + 90 min

    availability = _get_slot(client, barber.id, day, combo)
    slot = availability["slots"][0]
    response = client.post(
        f"{BASE}/appointments",
        json={
            "barber_id": barber.id, "service_ids": combo,
    ...
    short = _get_slot(client, barber.id, day, [services[3]["id"]])
    hour, minute = map(int, slot.split(":"))
    inside = f"{hour + 1:02d}:{minute:02d}"  # 60 min después del inicio
    assert inside not in short["slots"]


def test_booking_on_day_off_rejected(client, barbers):
    barber = barbers[0]
    day = next_day_off(barber)
    services = client.get(f"{BASE}/services").json()

    availability = _get_slot(client, barber.id, day, [services[0]["id"]])
...
def test_booking_outside_schedule_rejected(client, barbers):
    barber = barbers[0]
    day = next_working_date(barber, weeks_ahead=3)
    services = client.get(f"{BASE}/services").json()
    response = client.post(
        f"{BASE}/appointments",
        json={
...
def test_invalid_phone_rejected(client, barbers):
    barber = barbers[0]
    day = next_working_date(barber)
    services = client.get(f"{BASE}/services").json()
    response = client.post(
...
def test_unknown_tenant_404(client):
    response = client.get("/api/v1/public/no-existe/barbers")

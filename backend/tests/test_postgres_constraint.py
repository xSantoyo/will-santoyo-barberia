...
def _insert_appointment(conn, tenant_id, barber_id, starts, ends, status="confirmado"):
    conn.execute(
        text(
            """
            INSERT INTO appointments
                (tenant_id, barber_id, customer_name, customer_whatsapp,
                 starts_at, ends_at, status, daily_number, manage_code,
                 created_at, updated_at)
            VALUES (:tenant_id, :barber_id, 'Test', '+573000000001',
                    :starts, :ends, :status, 1, :code, now(), now())
            """
        ),
        {
            "tenant_id": tenant_id, "barber_id": barber_id,
...
def pg_fixture(pg_engine):
    ...
    yield tenant_id, barber_id
    with pg_engine.begin() as conn:
        conn.execute(text("DELETE FROM appointments WHERE tenant_id = :t"), {"t": tenant_id})
        conn.execute(text("DELETE FROM barbers WHERE tenant_id = :t"), {"t": tenant_id})
...
def test_db_rejects_overlap_even_bypassing_app(pg_engine, pg_fixture):
    ...
    tenant_id, barber_id = pg_fixture
    base = datetime(2030, 1, 15, 14, 0, tzinfo=timezone.utc)

    with pg_engine.begin() as conn:
        _insert_appointment(conn, tenant_id, barber_id, base, base + timedelta(minutes=45))

    # Solapamiento parcial directo por SQL → el constraint debe rechazarlo
    with pytest.raises(IntegrityError) as excinfo:
        with pg_engine.begin() as conn:
            _insert_appointment(
                conn, tenant_id, barber_id,
...
def test_db_allows_adjacent_and_cancelled(pg_engine, pg_fixture):
    ...
    with pg_engine.begin() as conn:
        ...
        _insert_appointment(
            conn, tenant_id, barber_id, base, base + timedelta(minutes=45),
            status="cancelado",
        )


def test_db_allows_same_time_other_barber(pg_engine, pg_fixture):
    tenant_id, barber_id = pg_fixture
    base = datetime(2030, 3, 5, 10, 0, tzinfo=timezone.utc)
    with pg_engine.begin() as conn:
        other_barber = conn.execute(
            text(
                "INSERT INTO barbers (tenant_id, name, created_at, updated_at) "
                "VALUES (:t, 'B2', now(), now()) RETURNING id"
            ),
            {"t": tenant_id},
        ).scalar()
        _insert_appointment(conn, tenant_id, barber_id, base, base + timedelta(minutes=45))
        _insert_appointment(conn, tenant_id, other_barber, base, base + timedelta(minutes=45))
